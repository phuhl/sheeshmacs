...
** Variables
#+begin_src emacs-lisp
  ;; Indents all content according to the corresponding headline.
  ;; Has slight issues with scrolling the buffer horizontally sometimes,
  ;; is it worth it in terms of estetics anyways (my opinion).
  ;; Seems to not be nessecary anymore since the use of org-beautify-theme
