...
(defun combobulate-navigate-down-list-maybe (&optional arg backwards)
...
When BACKWARDS is t the command \\[down-list] is mimiced with
negative argument."
  (interactive "^p")
    ...
    (with-navigation-nodes (:nodes combobulate-navigation-parent-child-nodes)
      (when-let ((navigable-node (combobulate--get-nearest-navigable-node))
                 ...
                 (targets (seq-filter
                           ...
                           (lambda (elem) (and elem (if backwards
                                                        (< elem (point))
                                                      (> elem (point)))))
                           (list (save-excursion (ignore-errors (down-list 1 nil) (point)))
                                 ...
                                 (ignore-errors (save-excursion
                                                  (if backwards
                                                    ...
                                                    (forward-sexp))
                                                  (point)))))))
        (when-let (target (apply (if backwards #'max #'min) targets))
          (goto-char target)
...
(defun combobulate-navigate-down-list-backward-maybe (&optional arg)
  (interactive "^p")
  (combobulate-navigate-down-list-maybe arg t))
