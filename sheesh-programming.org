...
#+BEGIN_SRC emacs-lisp
  ...
  (defun bind-custom-tab-hydra (custom-completions mode-name)
    (eval `(defhydra ,(intern (concat "hydra-autocomplete-" mode-name))
             (local-map ,(my-bind "Auto complete hydra") :hint nil :color blue :timeout 3)
             "Complete"
             ("c" copilot-accept-completion "Accept" :column "Copilot")
             ("w" copilot-accept-completion-by-word "Accept by Word" :color red)
             ("C-c" copilot-complete "Show completion")
    ...
    (define-key current-local-map (my-key "Auto complete hydra") (intern (concat "hydra-autocomplete-" mode-name "/body"))))
