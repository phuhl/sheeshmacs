...
#+BEGIN_SRC emacs-lisp :var keybindings=Keybindings
  (defvar global-keybindings nil "Keybinding lookup table from sheesh-emacs.org.")
...
#+BEGIN_SRC emacs-lisp
(sheeshmacs-load-file (concat sheeshmacs-dir "/src/elegant.el"))
(sheeshmacs-load-file (concat sheeshmacs-dir "/src/elegant-light.el"))
(sheeshmacs-load-file (concat sheeshmacs-dir "/src/elegant-dark.el"))
(sheeshmacs-load-file (concat sheeshmacs-dir "/src/org-beautify-theme.el"))
(sheeshmacs-load-file (concat sheeshmacs-dir "/src/md-beautify-theme.el"))
...
(use-package auto-dark
  ...
  :init (auto-dark-mode)
  :custom
  (auto-dark-themes '((elegant-dark org-beautify md-beautify) (elegant-light org-beautify md-beautify))))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Terminal: decode CSI u sequences for keys without standard escapes.
  ;; Requires matching sendInput actions in Windows Terminal settings.json.
  ;; See "Terminal Setup" section in sheeshmacs.org for the full
  ;; Windows Terminal configuration.
  (unless (display-graphic-p)
    (define-key input-decode-map "\e[44;5u" (kbd "C-,"))   ; 44 = ,
    (define-key input-decode-map "\e[46;5u" (kbd "C-."))   ; 46 = .
    (define-key input-decode-map "\e[59;5u" (kbd "C-;"))   ; 59 = ;
    (define-key input-decode-map "\e[39;5u" (kbd "C-'"))   ; 39 = '
    ;; C-M- combos: modifier 7 = Ctrl(4) + Meta(2) + 1
    (define-key input-decode-map "\e[106;7u" (kbd "C-M-j")) ; 106 = j
    )
#+END_SRC
...
(advice-add 'kill-buffer :around
  (lambda (orig-fn &optional buffer-or-name)
    (if (equal buffer-or-name "*scratch*")
        (bury-buffer)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra hydra-buffer-management
              ...
              ("2" (lambda () (interactive) (ensure-n-windows-balanced 2))  "Create two columns")
...
*** Disable savehist
Don't persist minibuffer history (e.g. `file-name-history') across sessions,
so old files don't pollute `counsel-find-file' candidates.
#+BEGIN_SRC emacs-lisp
  (savehist-mode -1)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    ...
    (use-package counsel
      ...
      ;; "/" enters the highlighted directory if it is one, but inserts
      ;; a literal "/" when typing path components like ../ or ~/
      (defun counsel-find-file-slash ()
        "Enter highlighted directory, or navigate up for ../"
        (interactive)
        (cond
         ;; ../ → go up a directory
         ((string= ivy-text "..")
         ...
         ;; highlighted item is a directory → enter it
         ((let ((current (ivy-state-current ivy-last)))
            (and current (string-suffix-p "/" current)))
          (ivy-alt-done))
         ;; default → insert literal /
         (t (self-insert-command 1))))
      (define-key counsel-find-file-map (kbd "/") 'counsel-find-file-slash)
...
*** Auto fill mode
  #+BEGIN_SRC emacs-lisp
  ;; Disable auto-fill in org/markdown (visual-line-mode handles wrapping).
  ;; Keep it for other text modes.
      ...
      (with-eval-after-load "ispell"
        ...
        (if ispell-program-name
            (progn
              (when (eq system-type 'windows-nt)
                (setenv "LANG" "en_US")
                ;; hunspell -D on Windows doesn't list available dicts,
                ;; so we manually register them for ispell.
                (setq ispell-hunspell-dict-paths-alist
                      '(("en_US" "C:/Hunspell/en_US.aff")
                        ("de_DE" "C:/Hunspell/de_DE.aff"))))
              (when (eq system-type 'gnu/linux)
          ...
          (warn "flyspell: hunspell not found in PATH. Install with: %s"
                (if (eq system-type 'windows-nt)
                    "choco install hunspell.portable"
                  "sudo apt install hunspell (Debian/Ubuntu) or sudo pacman -S hunspell (Arch)"))))))
...
#+BEGIN_SRC emacs-lisp
  (defvar languagetool-server-has-been-started nil)
...
#+BEGIN_SRC emacs-lisp
  ...
  a link to this file."
    (interactive)
    (let* ((tilde-buffer-filename
          (if (eq system-type 'gnu/linux)
              buffer-file-name
            (replace-regexp-in-string "/" "\\" (buffer-file-name) t t)))
           (_ (unless (file-exists-p (concat tilde-buffer-filename ".img"))
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Truncate lines when cursor is on a table (prevents ugly wrapping),
  ;; restore soft-wrap when cursor leaves the table.
  (defvar-local truncate-lines--in-table nil)
  (defun truncate-lines--table-toggle ()
    "Enable truncate-lines on table lines, disable elsewhere."
    (let ((in-table (or (and (derived-mode-p 'org-mode) (org-at-table-p))
                        (and (derived-mode-p 'markdown-mode 'gfm-mode)
                             (markdown-table-at-point-p)))))
      (unless (eq in-table truncate-lines--in-table)
        (setq truncate-lines--in-table in-table)
        (setq truncate-lines in-table)
        ;; visual-line-mode sets word-wrap; truncate-lines overrides it
        ;; but we need to toggle word-wrap too for clean display
        (setq word-wrap (not in-table)))))
  (add-hook 'org-mode-hook
    (lambda () (add-hook 'post-command-hook #'truncate-lines--table-toggle nil t)))
  ...
  (use-package markdown-mode
    ...
    (defun markdown-copy-formatted (r1 r2)
      "Copy region R1..R2 as formatted HTML to clipboard via pandoc.
Converts markdown to HTML, then puts it on the clipboard."
      (interactive "r")
      (let* ((text (buffer-substring-no-properties r1 r2))
             (html (with-temp-buffer
                     ...
                     (shell-command-on-region
                      (point-min) (point-max)
                      "pandoc -f gfm -t html" t t)
                     (buffer-string)))
             (tmpfile (make-temp-file "md-clip-" nil ".html" html))
             (ps1 (expand-file-name "src/html-clip-w32.ps1" sheeshmacs-dir))
             (proc (start-process "md-clip" "*md-clip*"
    ...
    (defun markdown-paste-html-clipboard ()
      "Paste HTML from clipboard as markdown markup via pandoc.
Falls back to regular yank if no HTML is on the clipboard."
      (interactive)
      (let* ((raw (string-trim
                   (shell-command-to-string
                    "powershell -command \"Get-Clipboard -TextFormatType Html\"")))
             (html (if (string-match "^Version:.*\n\\(?:[A-Za-z].*\n\\)*" raw)
                       (substring raw (match-end 0))
                     raw)))
        (if (or (string-empty-p html) (string-match-p "is not valid" raw))
            (yank)
          (insert (string-trim
                   (with-temp-buffer
                     (insert html)
    ...
    (defhydra hydra-markdown-mode-formatting (:hint nil) "MD Formatting/Headings"
      ...
      ("C" markdown-insert-gfm-code-block "Code block" :color blue)
    ...
    (defhydra hydra-markdown-mode (:hint nil)
      ...
      ("c" markdown-copy-formatted "Copy formatted" :color blue :column "Export/Import")
      ("y" markdown-paste-html-clipboard "Paste HTML as MD" :color blue))

    :config
    ;; Override HR rendering: replace text with a full-width space
    ;; (adapts to resize) and draw the line via face decoration.
    ;; Style matches the HR character: --- → strike-through,
    ;; ___ → underline, *** → overline.
    (advice-add 'markdown--fontify-hrs-view-mode :override
      (lambda (_hr-char)
        (let* ((hr-text (match-string 1))
               (face (cond
                      ((string-match-p "^_" hr-text)
                       '(:inherit markdown-hr-face :underline t))
                      ((string-match-p "^\\*" hr-text)
                       '(:inherit markdown-hr-face :overline t))
                      (t
                       '(:inherit markdown-hr-face :strike-through t)))))
          (add-text-properties
           (match-beginning 0) (match-end 0)
           `(face ,face font-lock-multiline t
                  display (space :align-to right-fringe))))))

    ;; Reveal markup on current line, re-hide when cursor moves away.
    ;; Works like org-appear does for org-mode.
    (defface markdown-code-block-separator
      '((t :underline t :extend t))
      "Face for code block opening delimiter lines (full-width underline).")

    (defvar-local markdown--reveal-prev-line nil
      "Line number where markup was last revealed.")

    (defun markdown--reveal-line (bol eol)
      "Strip hiding properties from a single line BOL..EOL."
      (remove-text-properties bol eol
        '(invisible nil display nil composition nil))
      ;; Remove code block delimiter underline when revealing
      (let ((face (get-text-property bol 'face)))
        (when (and (listp face) (memq 'markdown-code-block-separator face))
          (put-text-property bol (min (1+ eol) (point-max)) 'face
            (remq 'markdown-code-block-separator face))))
      (when (get-text-property bol 'markdown-hr)
        (remove-text-properties bol eol '(face nil))))

    (defun markdown--fix-code-block-delimiters ()
      "Fix code block delimiters across the visible window.
Font-lock makes entire delimiter lines invisible; we unhide each
line and hide just the backticks/tildes so the language name stays
visible and the line is navigable.  Opening delimiters get a
full-width underline.  Scans the whole visible window because
font-lock may extend its region beyond what jit-lock passes us."
      (let ((win-beg (if (get-buffer-window) (window-start) (point-min)))
            (win-end (if (get-buffer-window) (window-end nil t) (point-max))))
        (save-excursion
          (goto-char win-beg)
          (while (< (point) win-end)
            (let ((bol (line-beginning-position))
                  (eol (line-end-position)))
              (when (and (eq (get-text-property bol 'invisible) 'markdown-markup)
                         (save-excursion
                           (goto-char bol)
                           (looking-at "^[ \t]*\\(```+\\|~~~+\\)")))
                (let ((line-end (min (1+ eol) (point-max))))
                  (remove-text-properties bol line-end '(invisible nil))
                  (put-text-property (match-beginning 1) (match-end 1)
                    'display "")
                  ;; Add full-width underline to opening delimiters only.
                  ;; Opening = previous line is not code (i.e. not closing).
                  (let* ((prev-pos (max (point-min) (- bol 2)))
                         (prev-face (get-text-property prev-pos 'face))
                         (prev-is-code (if (listp prev-face)
                                           (memq 'markdown-code-face prev-face)
                                         (eq prev-face 'markdown-code-face))))
                    (unless (or prev-is-code
                                (if (listp (get-text-property bol 'face))
                                    (memq 'markdown-code-block-separator
                                          (get-text-property bol 'face))
                                  (eq (get-text-property bol 'face)
                                      'markdown-code-block-separator)))
                      (font-lock-append-text-property
                       bol line-end
                       'face 'markdown-code-block-separator))))))
            (forward-line 1)))))

    (defvar-local markdown--fix-in-progress nil
      "Guard against recursive invocation of markup fixup.")

    (defun markdown--fix-markup-display (beg end)
      "Fix hidden markup display between BEG and END.
Tables: replace invisible markup with spaces for column alignment.
Also fixes code block delimiters and re-reveals the cursor line."
      (when (and (bound-and-true-p markdown-hide-markup)
                 (not markdown--fix-in-progress))
        (setq markdown--fix-in-progress t)
        (unwind-protect
        (with-local-quit
        (with-silent-modifications
          ;; Fix code block delimiters (always scans visible window)
          (markdown--fix-code-block-delimiters)
          ;; Remove subscript display from strikethrough text (~~text~~).
          ;; The sub/superscript matcher sees ~text~ inside ~~text~~.
          (save-excursion
            (goto-char beg)
            (while (< (point) end)
              (let ((f (get-text-property (point) 'face)))
                (when (if (listp f)
                          (memq 'markdown-strike-through-face f)
                        (eq f 'markdown-strike-through-face))
                  (let ((d (get-text-property (point) 'display)))
                    (when (and (consp d) (consp (car d)) (eq (caar d) 'raise))
                      (remove-text-properties
                       (point) (1+ (point)) '(display nil))))))
              (forward-char 1)))
          (save-excursion
            ;; Fix tables within the jit-lock region
            (goto-char beg)
            (while (< (point) end)
                ...
                (let ((pos (line-beginning-position))
                  ...
                  (while (< pos eol)
                    (let ((inv  (get-text-property pos 'invisible))
                      ...
                      (if (or (eq inv 'markdown-markup)
                              (equal disp ""))
                          ;; Find the end of this contiguous hidden run
                          (let ((run-start pos))
                            (while (and (< pos eol)
                                        (let ((i (get-text-property pos 'invisible))
                                              (d (get-text-property pos 'display)))
                                          (or (eq i 'markdown-markup)
                                              (equal d ""))))
                              (when (eq (get-text-property pos 'invisible) 'markdown-markup)
                                (put-text-property pos (1+ pos) 'invisible nil))
                              (setq pos (1+ pos)))
                            ;; Replace entire run with correct number of spaces
                            (put-text-property run-start pos
                              'display (make-string (- pos run-start) ?\s)))
                        (setq pos (1+ pos)))))
                  ;; Ensure bold/italic faces render in tables
                  (let ((p (line-beginning-position)))
                    (while (< p eol)
                      (let ((f (get-text-property p 'face)))
                        (when (if (listp f)
                                  (memq 'markdown-bold-face f)
                                (eq f 'markdown-bold-face))
                          (font-lock-prepend-text-property
                           p (1+ p) 'face 'bold))
                        (when (if (listp f)
                                  (memq 'markdown-italic-face f)
                                (eq f 'markdown-italic-face))
                          (font-lock-prepend-text-property
                           p (1+ p) 'face 'italic)))
                      (setq p (1+ p))))))
              (forward-line 1))
            ;; Re-reveal cursor line if it was in the fontified region
            (when markdown--reveal-prev-line
              (goto-char (point-min))
              (forward-line (1- markdown--reveal-prev-line))
              (when (and (>= (point) beg) (< (point) end))
                (markdown--reveal-line
                 (line-beginning-position) (line-end-position)))))))
          (setq markdown--fix-in-progress nil))))

    (defun markdown--reveal-at-point ()
      "Reveal hidden markup on the current line, re-hide the previous line."
      (when (and (bound-and-true-p markdown-hide-markup)
                 (not markdown--fix-in-progress)
                 (derived-mode-p 'markdown-mode 'gfm-mode))
        (with-local-quit
          (let ((cur (line-number-at-pos)))
            (unless (eq cur markdown--reveal-prev-line)
              ;; Re-hide previous line by re-fontifying it
              (when markdown--reveal-prev-line
                (save-excursion
                  (goto-char (point-min))
                  (forward-line (1- markdown--reveal-prev-line))
              ...
              ;; Update tracked line BEFORE fix-display, so it knows
              ;; which line is current and won't re-reveal the old one
              (setq markdown--reveal-prev-line cur)
              ;; Fix visible tables (markup → spaces) and HRs
              (markdown--fix-markup-display
               (window-start) (window-end nil t)))
            ;; Always reveal current line
            ;; (jit-lock may re-hide during redisplay after post-command-hook)
            (with-silent-modifications
              (markdown--reveal-line
               (line-beginning-position) (line-end-position)))))))
    ...
    (add-hook
     'markdown-mode-hook
     #'(lambda ()
         ;; Ensure hide-markup is active in this buffer
         ...
         (add-hook 'post-command-hook #'markdown--reveal-at-point nil t)
         ;; Fix table and HR markup after font-lock runs
         (jit-lock-register #'markdown--fix-markup-display)
         ;; (languagetool-server-mode)
         )))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
    (defhydra hydra-C-semicollon (:hint nil :color blue)
      ...
      ("." flyspell-auto-correct-word "Autocorrect word")
