    ...
    (defun markdown--reveal-at-point ()
      ...
      (when (and (bound-and-true-p markdown-hide-markup)
        ...
        (let ((cur (line-number-at-pos)))
          ...
          ;; Always reveal current line — even on same-line edits
          ;; (jit-lock may re-hide markup during redisplay after
          ;; post-command-hook, so the jit-lock function also re-reveals)
          (with-silent-modifications
            (markdown--reveal-line
             (line-beginning-position) (line-end-position))))))
