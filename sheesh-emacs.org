...
Code block delimiters: unhide the line, hide just the backticks/tildes
so the language name stays visible and the line is navigable.
Tables: replace invisible markup with spaces for column alignment.
Also re-reveals the cursor line if font-lock just re-hid it."
          ...
          (save-excursion
            ...
            (while (< (point) end)
              (let ((bol (line-beginning-position))
                ...
                (cond
                 ;; Code block delimiter: entire line is invisible.
                 ;; Unhide the line, then hide just the backticks/tildes.
                 ((and (eq (get-text-property bol 'invisible) 'markdown-markup)
                       (save-excursion
                         (goto-char bol)
                         (looking-at "^[ \t]*\\(```+\\|~~~+\\)")))
                  (remove-text-properties bol (min (1+ eol) (point-max))
                    '(invisible nil))
                 ...
                 ;; Table row: replace invisible markup with spaces
