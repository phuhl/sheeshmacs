...
Automatically reveal emphasis markers, links, etc. when the cursor is
on them. Complements =org-hide-emphasis-markers= (set in elegant.el).
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Custom agenda view: regular agenda + unscheduled TODOs
  (setq org-agenda-custom-commands
        '(("n" "Agenda + Unscheduled TODOs"
           ((agenda "" nil)
            (tags-todo "TIMESTAMP>=\"<-7d>\"&TIMESTAMP<=\"<-1d>\""
                       ((org-agenda-overriding-header "Past Timestamped TODOs")))
            (alltodo ""
...
#+BEGIN_SRC emacs-lisp
  (defhydra hydra-org-mode-formatting (:hint nil) "Org Formatting/Headings"
    ("b" (org-toggle-emphasis ?*) "Bold" :column "Formatting")
    ("i" (org-toggle-emphasis ?/) "Italic")
    ("c" (org-toggle-emphasis ?=) "Code")
    ("v" (org-toggle-emphasis ?~) "Verbatim")
    ("u" (org-toggle-emphasis ?_) "Underline")
    ("~" (org-toggle-emphasis ?+) "Strike-through")

    ("h" org-insert-heading "Heading dwim" :column "Heading")
    ("1" (org-insert-heading) "Heading 1" :color blue)
    ("2" (progn (org-insert-heading) (org-demote)) "Heading 2" :color blue)
    ("3" (progn (org-insert-heading) (org-demote) (org-demote)) "Heading 3" :color blue)
    ("4" (progn (org-insert-heading) (org-demote) (org-demote) (org-demote)) "Heading 4" :color blue)

    ("C-g" hydra-org-mode/body :color blue :column "Other"))

  (defhydra hydra-org-mode (:hint nil :color blue)
    "Org"
    ("f" hydra-org-mode-formatting/body "Formatting/Headings" :column "Edit")
    ("t" org-toggle-list-checkbox "Toggle checkbox")
    ("I" org-screenshot "Insert Screenshot from Clipboard")

    ("c" ox-clip-formatted-copy "Copy formatted" :column "Export/Import")
    ("y" org-paste-html-clipboard "Paste HTML as Org")
...
#+BEGIN_SRC emacs-lisp
  (defun org-toggle-emphasis (char)
    "Toggle emphasis CHAR at point or on region.
  If region is active, wrap it with CHAR.
  If point is inside emphasis of the matching type, remove it.
  Otherwise, insert new emphasis markers."
    (interactive)
    (if (use-region-p)
        (org-emphasize char)
      (let* ((elem (org-element-context))
        ...
        (if (eq type expected)
            ;; Remove emphasis: delete closing marker first, then opening
  ...
  (defun org-toggle-list-checkbox ()
    "Toggle checkbox on current list item.
  If item has no checkbox, add [ ].  If it has one, remove it."
    (interactive)
    (save-excursion
      (beginning-of-line)
      (cond
       ;; Has checkbox — remove it
       ((looking-at "\\([ \t]*\\(?:[-+*]\\|[0-9]+[.)]\\)\\) \\[[ X-]\\] ")
        (replace-match "\\1 "))
...
Pastes HTML from the system clipboard as org-mode markup using pandoc.
Falls back to plain text if no HTML is available.

#+BEGIN_SRC emacs-lisp
  (defun org-paste-html-clipboard ()
    "Paste HTML from clipboard as org-mode markup via pandoc.
...
#+BEGIN_SRC emacs-lisp
  (use-package ox-clip :ensure t
    ...
    ;; On Windows, override ox-clip's synchronous shell-command-on-region
    ;; with an async start-process approach to avoid clipboard deadlock
    (when (eq system-type 'windows-nt)
      (defvar ox-clip-w32-ps1
        (expand-file-name "src/html-clip-w32.ps1" sheeshmacs-dir))

      (advice-add 'ox-clip-formatted-copy :around
        (lambda (orig-fun r1 r2 &optional subtreep)
          (if (null subtreep)
          ...
          (save-window-excursion
            (let* ((html-buf (cond
                               ((equal major-mode 'org-mode)
                                (org-export-to-buffer 'html "*Formatted Copy*"
                                  nil subtreep (not subtreep) t))
                               ((derived-mode-p 'markdown-mode 'gfm-mode)
                                (let ((buf (generate-new-buffer "*Formatted Copy*"))
                                      (text (buffer-substring-no-properties r1 r2)))
                               ...
                               (t
                                (let ((html (htmlize-region-for-paste r1 r2))
                                     (buf (generate-new-buffer "*Formatted Copy*")))
                                  (with-current-buffer buf (insert html))
                                  buf))))
                   (tmpfile (make-temp-file "ox-clip-" nil ".html"
                              (with-current-buffer html-buf (buffer-string))))
                   (proc (start-process "ox-clip" "*ox-clip*"
                           "powershell" "-NoProfile" "-STA"
                           "-ExecutionPolicy" "Bypass"
                           "-File" ox-clip-w32-ps1
                           "-InputFile" tmpfile)))
