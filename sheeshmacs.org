...
External programs needed for full feature support. Run these commands
...
#+BEGIN_SRC powershell
choco install git diffutils pandoc ripgrep hunspell.portable nodejs python rustup.install plantuml irfanview languagetool temurin -y
#+END_SRC

| Program       | Purpose                                    |
|---------------+--------------------------------------------|
| git           | Version control, Magit                     |
| diffutils     | Ediff diff/merge                           |
| pandoc        | HTML/Org/Markdown format conversion        |
| ripgrep       | Fast search (counsel-rg)                   |
| hunspell      | Spell checking                             |
| nodejs        | ESLint, Prettier, JS/TS tooling            |
| python        | Org-mode code blocks                       |
| rustup        | Rust development + rust-analyzer           |
| plantuml      | UML diagrams in org-mode                   |
| irfanview     | Paste screenshots from clipboard           |
| languagetool  | Grammar checking                           |
| temurin       | Java runtime (needed by LanguageTool)      |

** Post-install steps

*** Hunspell dictionaries
Download =en_US= and =de_DE= dictionaries (.aff and .dic files) from
[[https://cgit.freedesktop.org/libreoffice/dictionaries/tree/]] and
place them in =C:\Hunspell\=.
...
*** Verify PATH
The =pathes.el= file (generated on first start) must include paths to
=diffutils=, =git=, and =hunspell=. See the Pathes section below.

** Terminal Setup (Windows Terminal)

When running Emacs in a terminal, some key combinations (=C-.=, =C-,=,
=C-;=, =C-'=) have no standard escape sequences. Sheeshmacs includes
CSI u decoding for these keys, but Windows Terminal must be configured
to send the matching sequences.

Add the following to your Windows Terminal =settings.json=
(Settings → Open JSON file), inside the ="actions"= array:

#+BEGIN_SRC json
{ "keys": "ctrl+,", "command": { "action": "sendInput", "input": "\u001b[44;5u" } },
{ "keys": "ctrl+.", "command": { "action": "sendInput", "input": "\u001b[46;5u" } },
{ "keys": "ctrl+;", "command": { "action": "sendInput", "input": "\u001b[59;5u" } },
{ "keys": "ctrl+'", "command": { "action": "sendInput", "input": "\u001b[39;5u" } },
{ "keys": "ctrl+alt+j", "command": { "action": "sendInput", "input": "\u001b[106;7u" } }
#+END_SRC

The format is CSI u: =ESC [ <ascii-code> ; <modifier> u= where the
modifier 5 means Ctrl. This is the same protocol used by kitty and
other modern terminals.

| Key   | ASCII | Escape Sequence  | Sheeshmacs Function        |
|-------+-------+------------------+----------------------------|
| =C-,= |    44 | =\e[44;5u=       | Previous buffer            |
| =C-.= |    46 | =\e[46;5u=       | Next buffer                |
| =C-;= |    59 | =\e[59;5u=       | Main hydra menu            |
| =C-'= |    39 | =\e[39;5u=       | Cycle quotes (JS/TS)       |
| =C-M-j= |   106 | =\e[106;7u=      | Ivy immediate done         |
...
#+BEGIN_SRC emacs-lisp
  ...
  (when (eq system-type 'windows-nt)
    ;; Chocolatey binaries (diff, hunspell, ripgrep, pandoc, etc.)
...
Helper for byte-compiling and loading individual .el files.
  #+name: sheeshmacs-load-file
  #+BEGIN_SRC emacs-lisp
    (defun sheeshmacs-load-file (file)
      "Byte-compile FILE if needed, then load the compiled version.
    FILE should be an absolute path to an .el file.
    Respects the `no-byte-compile' file-local variable."
      (let* ((elc (concat file "c"))
             (no-compile (with-temp-buffer
                           (insert-file-contents file)
                           (goto-char (point-max))
                           (search-backward "no-byte-compile: t" nil t))))
        (when (and (not no-compile)
                   (or (not (file-exists-p elc))
                       (file-newer-than-file-p file elc)))
          (ignore-errors (byte-compile-file file)))
        (load-file (if (file-exists-p elc) elc file))))
  #+END_SRC

Function for loading other parts of sheeshmacs.
Tangles .org to .el (only when needed), byte-compiles, then loads.
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    add (sheeshmacs-load \"org\") to your configuration."
      (let* ((org-file (expand-file-name (if (string-match "sheesh-.+\.org" file)
                                             file
                                           (format "sheesh-%s.org" file))
                                         sheeshmacs-dir))
             (org-file (if header-or-tag
                           (let* ((base (file-name-nondirectory org-file))
        ...
        ;; Tangle .org → .el only when needed
        (when (or (not (file-exists-p el-file))
                  (file-newer-than-file-p org-file el-file))
          (require 'ob-tangle)
          (org-babel-tangle-file org-file el-file "emacs-lisp"))
        ;; Byte-compile if needed, then load
        (sheeshmacs-load-file el-file)))
  #+END_SRC
